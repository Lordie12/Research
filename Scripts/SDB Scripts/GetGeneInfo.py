...
sort_order = ['desc', 'asc']
year = ('1980', '1990')
imdb_url = 'http://www.imdb.com/search/title?at=0&release_date=1990,2000&sort=user_rating,desc&start=1&title_type=feature&country=us'
movie_url = 'http://www.imdb.com'
binfo_url = 'business?ref_=tt_dt_bus'
file_url = '/Users/Lanfear/Desktop/Research/CLuuData/TimeWindowResults/moviewindow' + str(year[0]) + '-' + str(year[1]) + '.csv'

'''
/-----------------------------------------------------
Method to modify the search URL using the given params
-----------------------------------------------------/
'''
...
'''
/-----------------------------------------------------
Method to get movie title from html
-----------------------------------------------------/
'''
...
def get_rating_from_soup(soup):
    rating = soup.find('div', attrs={'class': 'titlePageSprite star-box-giga-star'})
...
def get_budget_from_soup(soup):
    cond = soup.findAll('h4', attrs={'class': 'inline'}, text=re.compile(r'Budget:'))  
